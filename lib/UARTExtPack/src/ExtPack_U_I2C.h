 ...
 */
static inline ext_pack_error_t set_ExtPack_I2C_partner_adr(unit_t unit, uint8_t slave_id) {
...
static inline ext_pack_error_t receive_ExtPack_I2C_data(unit_t unit) {
    return send_to_ExtPack(_set_ExtPack_access_mode(unit, 10), 0x00);
}
 ...
 */
static inline ext_pack_error_t send_ExtPack_I2C_String(unit_t unit, const uint8_t* data, uint16_t send_delay_us, uint8_t max_attempts, uint16_t retry_delay_us) {
    return send_String_to_ExtPack(_set_ExtPack_access_mode(unit, 00), data, send_delay_us, max_attempts, retry_delay_us);
}
