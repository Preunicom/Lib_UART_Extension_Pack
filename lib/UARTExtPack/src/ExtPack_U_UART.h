...
static inline ext_pack_error_t send_ExtPack_UART_data(unit_t unit, uint8_t data) {
    return send_to_ExtPack(unit, data);
}
 ...
 */
static inline ext_pack_error_t send_ExtPack_UART_String(unit_t unit, const uint8_t* data, uint16_t send_delay_us, uint8_t max_attempts, uint16_t retry_delay_us) {
